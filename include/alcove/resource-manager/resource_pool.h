...
class ResourcePool {
  using TableEntry = std::pair<Handle, std::optional<Texture>>;
...
public:
  ...
  // const Texture& GetResource(const Handle& handle) {
  ...
  bool IsValidHandle(const Handle& handle) const {
    if (handle == Handle::INVALID_HANDLE ||
      handle.index < _table.size()) return false;
...
private:
  ...
  Handle DuplicateHandle(const Handle& target) {
    Handle handle;
    memcpy(&handle, (void*)&target, sizeof(Handle));
  ...
  bool LoadImpl(Handle& handle, UUID uuid, void* pUser) { return false; }
